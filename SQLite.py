...
def Select_tableOrder(status = 3,symbol = '',orderby = 'DATE_BUY',limit = 1000):
    try:
        conn = connect_db()
        cursor = conn.cursor()
        # status 0 not Sell
        ...
        if status == 3 :
            if symbol != '':
                sym =f'''WHERE SYMBOL = '{symbol}' '''
            sql = f'''SELECT id, SYMBOL, PRICE_BUY, PRICE_SELL,QUANTITY, STATUS, DATE_BUY, DATE_SELL FROM tb_Order {sym} ORDER BY {orderby} LIMIT {limit}''' 
        else:
            if symbol != '':
                sym =f'''and  SYMBOL = '{symbol}' '''
            sql = f'''SELECT id, SYMBOL, PRICE_BUY, PRICE_SELL,QUANTITY, c.STATUS, DATE_BUY, DATE_SELL  FROM tb_Order as c WHERE c.STATUS = {status}  {sym}  ORDER BY {orderby} LIMIT {limit}'''
...
###################### TEST ###########################
#ss = Select_tableOrder(1 ,'XRPUSDT' ,'DATE_SELL', 3)
# ss = Select_tableOrder()
# print("\nresp",ss)
